...
def analyze_corr_AC_interpolate(config, corr_AC, window=window_exp, ntime=1):
    ...
    omegas = np.linspace(0, en_range, tnr*ntime)
    ac_prefactor = 4. * config.sample['nr_orbitals'] \
        / config.sample['area_unit_cell'] \
        / config.sample['extended']

    def get_acv(omega, corr_AC):
        if omega == 0.0:
            acv =  np.array([0., 0., 0., 0.])
        else:
            acv = 0.
            ks = np.arange(tnr)
            acv_ks = 2* window(ks+1, tnr) * np.sin(omega * ks * t_step) * corr_AC.imag
            acv = ac_prefactor * t_step * np.sum(acv_ks, axis=1) * (np.exp(-beta * omega) - 1) / omega
        return acv

    AC = np.array([get_acv(omega, corr_AC) for omega in omegas])
    # correct for spin
    if config.generic['correct_spin']:
        AC = 2. * AC
    return omegas, AC
...
def get_dckb(sample, config):
    ...
    conductivity = fortran_f2py.cond_from_trace(
        mu_mn, energies, sample.rescale, beta, ne_integral,
        fermi_precision, kbdc_prefactor)
